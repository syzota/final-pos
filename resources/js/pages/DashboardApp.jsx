...
const TITLES = {
  dashboard: ['Beranda', 'Ringkasan terpadu data kesehatan dan rekapitulasi Posyandu'],
  kesehatan: ['Pencatatan Kesehatan', 'Input hasil pemeriksaan 4 kelompok sasaran warga'],
  pengaduan: ['Formulir & Pengaduan', 'Identifikasi kebutuhan 5 bidang non-kesehatan & aspirasi warga'],
  'kelola-makanan': ['Kelola Data Makanan', 'Daftar referensi kalori makanan lokal'],
  profil: ['Profil & Sarana Posyandu', 'Informasi profil dan fasilitas posyandu binaan'],
  daftar: ['Daftar 9 Posyandu', 'Referensi seluruh Posyandu di Desa Loa Duri Ulu'],
  artikel: ['Artikel & Berita', 'Kelola informasi dan edukasi kesehatan untuk warga'],
  kelolawarga: ['Kelola Warga', 'Manajemen data kartu keluarga & sasaran posyandu'],
  'puskesmas-dashboard': ['Laporan Bulanan Kesehatan', 'Tinjau dan ekspor laporan bulanan Posyandu'],
  'superadmin-dashboard': ['Transparansi Progres Pelaporan', 'Status rekapitulasi berkala 9 Posyandu'],
  'superadmin-analitik': ['Dashboard Analitik SPM', 'Analisis tren kesehatan & indikator desa'],
  'superadmin-ekspor': ['Ekspor Gabungan 9 Posyandu', 'Unduh rekapitulasi data gabungan'],
  'warga-anak': ['Rapor Kesehatan Keluarga', 'Riwayat tumbuh kembang balita dan anggota keluarga'],
  'warga-kalkulator': ['Kalkulator Gizi Mandiri', 'Perhitungan gizi dan riwayat konsumsi kalori'],
  'pencatatan-kegiatan': ['Laporan 13 Poin Kegiatan', 'Formulir pencatatan kegiatan berkala'],
  'data-umum': ['Data Umum Posyandu', 'Statistik kependudukan sasaran posyandu'],
  'data-tambahan': ['Data Tambahan', 'Rekapitulasi sasaran khusus ibu hamil & nifas'],
  'admin-arsip': ['Arsip Laporan Posyandu', 'Riwayat dan arsip digital laporan bulanan'],
  'ganti-password': ['Ganti PIN / Sandi', 'Perbarui kode akses akun Anda secara berkala'],
};
    ...
    <div id="app" style={{ display: 'block' }}>
      ...
      <div className="shell">
        ...
        <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`} id="sidebar">
          {/* Brand Header */}
          <div className="brand" style={{ flexShrink: 0, padding: '20px 16px 12px', borderBottom: '1px solid #f1f5f9' }}>
            <div className="mark" style={{ width: '38px', height: '38px', borderRadius: '10px', overflow: 'hidden' }}>
              <img src={logo} alt="Logo Posyandu" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
            <div>
              <div className="brand-name" style={{ fontSize: '15px', fontWeight: 800, color: '#0f172a' }}>
                Posyandu Loa Duri Ulu
              </div>
              <div className="brand-sub" style={{ fontSize: '11.5px', color: '#64748b' }}>
          ...
          <div className="sidebar-user-card" style={{ flexShrink: 0 }}>
            <div style={{ width: '34px', height: '34px', borderRadius: '50%', backgroundColor: 'var(--primary-teal, #008080)', color: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '13px' }}>
              <User size={18} />
            </div>
            <div style={{ minWidth: 0, flex: 1 }}>
              ...
              <div style={{ fontSize: '11px', color: '#008080', fontWeight: 600 }}>
                {getRoleLabel()}
              </div>
            </div>
          </div>
          ...
          <div id="navContainer">
            {NAV[role]?.map(group => (
              <React.Fragment key={group.group}>
                <div className="nav-group-label" style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', color: '#94a3b8', padding: '12px 10px 4px', letterSpacing: '0.05em' }}>
                  {group.group}
                </div>
                {group.items.map(item => (
                  <div
                    ...
                    style={{
                      ...
                      padding: '10px 14px',
                      borderRadius: '10px',
                      fontSize: item.id === 'dashboard' ? '15px' : '14px',
                      fontWeight: item.id === 'dashboard' ? 700 : 600,
                      cursor: 'pointer',
                      marginBottom: '2px'
                    }}
                  >
